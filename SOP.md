...
| **C1. 官方網站（月班表）** | 月班表 | **前月月底** | c15 誠陽、c16 康澤、c26 新店高美 |
...
| c26 新店高美泌尿科 | https://www.kaomei.com.tw/hours.asp?id=37 | 周孟翰 | 新開診所，暫列官網月班表；目前可 curl 靜態 HTML 表格 |
...
### c26 新店高美：靜態 HTML 表格解析

新店高美暫定為官網月班表，不視為永久固定班表。原因：新開診所，官網門診排版可能在穩定前改版。

目前最穩定 touch point：
- URL 固定：`https://www.kaomei.com.tw/hours.asp?id=37`
- `curl` 可直接取得完整 HTML，無需瀏覽器、無需 OCR
- 目標為頁面內含 `MON` ~ `SUN` 表頭，且列名為 `上午` / `下午` / `晚上` 的表格
- parser 不依賴 inline style、width、line number

目前解析到的週班表：
- 上午 09:00–12:00：週一到週六 周孟翰
- 下午 14:30–17:30：週三到週五 周孟翰
- 晚上 18:30–20:30：週一、週二、週三、週五 周孟翰
...
# 產生目前月份到下個月底 sessions，並寫入 schedules.json
python3 scraper/kaomei_scraper.py --start-date 2026-06-15 --months 2 --update-schedules
```

每月更新時先看 parser 是否仍能辨識 table 結構。若官網改成圖片、iframe、或多表格，不要沿用舊 parser 硬吃資料，先重新判斷 touch point。
...
□ 類型 C1（官網月班表：c15/c16/c26）：截圖或 parser → 讀取整月 → 新增下月 sessions（不刪本月）
